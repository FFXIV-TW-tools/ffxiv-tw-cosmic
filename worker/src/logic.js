 ...
 *
 * 上界 5（2026-08-03 由 15 收緊）：**遊戲能給的預警本來就只有約 5 分鐘**——
 * 實測兩筆（孢子霧 5 分 40 秒、流星雨 5 分 15 秒），而玩家唯一的資訊來源就是那則預兆通告
 * ⇒ 沒有任何管道能讓人知道 10 分鐘或 15 分鐘後的事。收得比這寬只是收噪音，
 * 而且和 UI 給的選項（最多 5 分鐘）不一致。
 *
 * 原本放寬到 15 的理由是「只有單一觀察，別拿它當硬上限」。現在有兩筆、且兩筆都落在 5 分半內，
 * 更重要的是**這不是在猜遊戲的行為，是在描述通報者能知道什麼**——他不可能比通告更早知道。
 * 若日後遊戲真的改成更早預告，症狀會是通報被退（400、看得見），不是靜默漏掉。
 */
export const MAX_LEAD_MINUTES = 5;
