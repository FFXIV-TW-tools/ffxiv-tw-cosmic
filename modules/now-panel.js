 ...
 *
 * ⚠ 2026-08-01 曾因為 ET 02:00–04:00 面板空白而把基礎任務也放進來——**那是治標**。
 * 根因是分類判準錯（#400 rank A 帶 ET 條件被判成基礎，實際在遊戲的臨時分頁）。
 * 判準修好後 63 個有條件的任務全是臨時任務，面板已收回只收臨時。
 * 教訓：畫面少東西時先問「資料判對了嗎」，不要先放寬篩選——那會把錯的分類蓋掉。
 *
 * ## 「現在」是保證，「下一個」是預測
...
/** 臨時任務的子標籤（Addon#16872/16873）。子標籤是臨時分頁的定義，基礎任務不會有。 */
...
export function createNowPanel(root, { windows, missions, conditions, jobs, forecaster, onJump }) {
   ...
   *
   * **緊急任務排除**：2026-07-31 Owner 在靈風時段實測，本站列的 8 個裡有 3 個是緊急任務
   * （製作火焰噴射器的零件／調查孢子霧對植被的影響／採集有益菌類），遊戲內都沒有。
   * ⚠️ 當時寫的理由是「天候條件只是**必要**條件，還要等伺服器推播」——**那個理由是錯的**：
   * 2026-08-02 定性後，33 個 `critical` 的 `conds` **全部是空的**（現況實測 0/33），
   * 緊急任務跟天候條件根本沒有關係，會列出來純粹是當時 c14/c15 判錯欄位。
   * 下面的 `class !== 'temporary'` 對現在的資料是 no-op，留著是防日後改版又混進來。
   *
  ...
  function relevant(m) {
     ...
     * 面板名叫「臨時任務」就只放臨時任務。
     *
     * ⚠ 2026-08-01 一度改成「基礎任務也收」，那是**錯的修法**：當時 ET 02:00–04:00 面板空白，
     * 根因是**分類判準錯了**（#400 rank A 帶 ET 條件被判成基礎，實際在遊戲的臨時分頁），
     * 不是面板篩太緊。判準修好後（見 TcCosmicSheets.MissionClass），63 個有條件的任務
     * **全部**都是臨時任務，這個過濾器對它們是 no-op ⇒ 收回來，不要用放寬篩選去蓋資料的錯。
     * 留著它是為了萬一日後真出現「有條件的基礎任務」時，這裡不會默默把它混進來。
     */
  ...
  function collapseByRank(list) {
    const key = (m) => `${m.jobs[0]}|${m.conds[0] ?? 0}`;
    const best = new Map();
    for (const m of list) {
   ...
   *
   * 用 350 秒（＝2 艾奧傑亞小時）當掃描步長：ET 視窗邊界正好是 350 的倍數，
  ...
  /**
   * 緊急事件的告誡**只在特殊天氣（靈風／月塵）進行中才出現**——不是因為那時比較容易出事件
   * （見下），而是因為**天氣限定任務只綁這兩個天氣**（20 個靈風＋21 個月塵，晴朗零個）⇒
   * 只有這兩段時間裡「被覆蓋」才會讓上面的清單失效。晴朗時這段沒有指涉對象。
   *
   * 為什麼這件事非講不可：緊急天氣（磁暴／流星雨／孢子霧）會**覆蓋**當前天候 ⇒
   * 被覆蓋的那段時間，上面每一筆天氣限定任務都是錯的。
   * Owner 2026-07-31 實測：靈風進行中突然轉孢子霧，限時任務整批消失、改出緊急事件。
   *
   * ⚠️ 2026-08-06 更正：舊文案寫「現在是月塵，**隨時可能觸發**緊急事件」＋「本站也**偵測不到**
   * 它正在發生」，兩句都是錯的。
   * · **與天氣無關**：173 筆實測通報回推當下的演算法天氣＝晴朗 102／月塵 35／靈風 36，
   *   對照**同一段期間實際的天氣時段分布**（358／102／90）χ²=3.35（df=2，臨界 5.99）⇒ 與隨機取樣無異。
   *   **多數是從晴朗直接轉過去的。** ⚠️ 基準不可用機率表的 70/15/15——9 天只有 550 段，
   *   天氣本身就會偏離長期機率，用理論值會算出假的顯著（同一份資料 χ² 會變成 10.06）。
