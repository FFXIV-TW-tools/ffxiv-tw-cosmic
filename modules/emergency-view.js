 ...
 *
 * 往後只到 5 分鐘：遊戲的預兆通告只提前約 5 分鐘，而玩家唯一的資訊來源就是它
 * ⇒ 沒有管道能知道更久之後的事。後端上限同為 5。
 * ⚠️ 2026-08-11 更新依據：原本引的是最早兩筆手動觀測（5:15／5:40）。現在有插件量的
 * `warnedAt→startAt` **18 筆，其中 17 筆落在 4 分 38–41 秒**（全距 3 秒）⇒ 5 分鐘的上限仍然成立。
 * 那唯一的離群值是 9 分 3 秒——**它反而超出這個選單的範圍**，代表偶爾會有人無法如實通報，
 * 這是已知的取捨（放寬上限會讓所有人都能填一個沒有依據的大數字）。
 */
/**
...
/** 一台伺服器一列。沒有事件的也要列出來——「查過了，沒有」跟「不知道」是兩回事。 */
  function row(world, ev, now) {
    ...
    if (warnOnly) {
      // **不編造倒數**。⚠️ 2026-08-11 更新理由：樣本已經不只一筆了（18 筆，其中 17 筆
