...
**2026-08-06：從單一觀察升級為統計結論**（n=71，`/history` 的 `status=active` 通報，天氣是時間
的純函數 ⇒ 每筆的底層演算法天氣都能離線回推）：

| | 晴朗 | 月塵 | 靈風 |
|---|---|---|---|
| 事件開始時的演算法天氣 | 45（63.4%）| 13（18.3%）| 13（18.3%）|
| 基準機率 | 70% | 15% | 15% |

χ²=1.48（df=2，臨界 5.99）⇒ **與隨機取樣無異**。**45 筆是從晴朗直接轉過去的**，
不存在「先靈風／月塵再轉」這條路。另兩條佐證：
...
- **真正的形狀是 per-server 計時器**：同伺服器相鄰事件間隔 64 筆中，47 筆落在 **214–311 分**
  （中位 254 分 ≈ 4 小時 14 分），其餘 17 筆全是該值的近整數倍（1.8×／2.0×／3.0×…）＝那幾輪沒人通報。
...
| `modules/emergency-*.js` | `node tests/run-all.mjs`（**7 個測試檔**，含 `emergency-view.test.mjs`＝4 條時序斷言）＋本機 `wrangler dev` ＋瀏覽器走一次通報→附議→訂閱 | 測試全綠；console 零 error；後端關掉時該分頁降級為唯讀、其他分頁不受影響。⚠️ **前景輪詢時序量不到**——自動化開的分頁本身就 `document.hidden`（見鐵則 §5 末），要驗間隔得用真人分頁 |
...
<!-- TEST-BASELINE cmd="node tests/run-all.mjs" match="(\d+)/\d+ 測試檔通過" expect="7" label="前端 run-all" -->
